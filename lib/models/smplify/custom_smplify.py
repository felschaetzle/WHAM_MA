...
class CustomSMPLify():
    ...
    def fit(self, init_pred, keypoints, bbox, gt_extrinsics, cam_intrinsics):
        ...
        optim_params = [params[2], params[3]]
        ...
        print(f"Final loss: {loss.item():.1f}")
...
def progressive_global_translation_optimization(init_pred, keypoints, bbox,
    ...
    transl_world, poses_root_world = W_MPJPE_align(cam, bbox, res, cam_intrinsics, smpl, device, pose, betas, transl_wham_world, poses_root_wham_world, gt_extrinsics, window_size)
    ...
    trans_world_raw = init_pred['trans_world'][:window, :].cpu()
    ax.scatter(gt_trans[:, 0], gt_trans[:, 1], gt_trans[:, 2], c='r', marker='o', label='GT')
...
def W_MPJPE_align(cam, bbox, res, cam_intrinsics, smpl, device, pose, betas, transl_wham_world, poses_root_wham_world, gt_extrinsics, window_size):
    ...
    for window in range(0, n, window_size):
        ...
        output = smpl.forward_align(pose[:,window:window+1], betas[:,window:window+1], trans_opt=transl_wham_world[window:window+1], global_orient_opt=poses_root_wham_world[window:window+1])
